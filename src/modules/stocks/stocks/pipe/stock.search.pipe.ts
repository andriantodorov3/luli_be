...
export class StockSearchPipe implements PipeTransform {
    ...
    transform(value: any, metadata: ArgumentMetadata) {
        ...
        if (value.start_time < configStartTime) {
            error = true;
            errorsResponse.start_time.push(`Start time cannot be prior ${new Date(<number>configStartTime*1000).toISOString()}`)
        }

        if (value.end_time > configEndTime) {
            error = true;
            errorsResponse.end_time.push(`Start time cannot be past ${new Date(<number>configEndTime*1000).toISOString()}`)
        }

        if (value.start_time == value.end_time) {
            error = true;
            errorsResponse.start_time.push(`Start time cannot be the same as end time `)
        }
